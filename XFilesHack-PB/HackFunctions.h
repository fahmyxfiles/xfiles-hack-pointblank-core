...
bool IsInBattle()
{
	DWORD dwResult = oReadBaseValue(AddyStruct->GameCharaBase.Base);
	if(!IsBadReadPtr((void*)dwResult,4))return true;
	return false;
}
...
DWORD hookE8(PDWORD target, PDWORD myFunction)
{
	DWORD Jmpto=((DWORD)(myFunction)-(DWORD)target)-5;
	DWORD a;
	VirtualProtect(target, 5, PAGE_EXECUTE_READWRITE, &a);
...
tVirtualProtectEx oVirtualProtectEx;

void InitBypassKernelFunction()
{
	DWORD dwVirtualProtectEx = (DWORD)GetProcAddress(GetModuleHandle("KERNELBASE.dll"), "VirtualProtectEx");
	if(!dwVirtualProtectEx)
	{
		dwVirtualProtectEx = (DWORD)GetProcAddress(GetModuleHandle("kernel32.dll"), "VirtualProtectEx");
	}
	virtualProtect_Back = dwVirtualProtectEx + 5;
	oVirtualProtectEx = (tVirtualProtectEx)codeCave_virtualProtectEx;
}

DWORD DetourB8( DWORD SrcVA, DWORD DstVA, DWORD Size )
{
	DWORD DetourVA, dwProtect, dwProtect2, i;
	...
	if ( SrcVA && DstVA && Size >= SIZEOF_MOVEAX_JMPEAX )
	{
		...
		if ( DetourVA && oVirtualProtectEx((HANDLE)-1, (VOID*)SrcVA, Size, PAGE_READWRITE, &dwProtect ) )
		{
			...
			*(DWORD*)( SrcVA + 1 ) = ( DstVA );
			*(WORD*)( SrcVA + 5 ) = 0xE0FF;
			oVirtualProtectEx((HANDLE)-1,  (VOID*)SrcVA, Size, dwProtect, &dwProtect );
			oVirtualProtectEx((HANDLE)-1,  (VOID*)DetourVA, Size + 
			SIZEOF_MOVEAX_JMPEAX, dwProtect2, &dwProtect2 );
			return DetourVA;
		}
	}
return (0);
}
void hexDumpFile (char *desc, void *addr, int len, FILE* pFile) {
    int i;
